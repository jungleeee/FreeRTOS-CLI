...
#define     debugPrintInfo( s, ... )    { if(s) { \
                                            if(cli_print_malloc() == TRUE) { \
                                                if(snprintf(pcCliOutputString, cli_MAX_TX_QUEUE_LEN, __VA_ARGS__) > cli_MAX_TX_QUEUE_LEN) { cli_print_info(); \
                                                    if(cli_print_malloc() == TRUE) { \
                                                        sprintf(pcCliOutputString, pcWarningMessageString); cli_print_info(); \
                                                    } \
                                                }else cli_print_info(); \
                                            } \
                                        } }
...
extern char * pcCliOutputString;
extern volatile CLI_DEBUG_PEINT_STATUS xCliDebugStatus;

/* Exported functions ----------------------------------------------------------*/
extern unsigned int cli_print_malloc(void);//cli_task.c
